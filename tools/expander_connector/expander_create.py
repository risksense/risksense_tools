...
class expander_create:
    ...
    def __init__(self):
        ...
        self.expander_username=config['expander_username']
        self.expander_password=config['expander_password']
        self.connectorurl=config['connector_url']
        self.scheduletype=config['schedule_type']
        self.scheduleenabled=config['schedule_enabled']
        self.hourofday=config['hourOfDay']
        self.autourba=config['autoUrba']
        self.rs.set_default_client_id(self.client_id)
        try:
            connector_details=self.rs.connectors.create_expander(self.connector_name,self.connectorurl,self.scheduletype,self.networkid,self.expander_username,self.expander_password,hour_of_day=self.hourofday,)
