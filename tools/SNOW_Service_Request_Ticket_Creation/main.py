...
|
|  Name        :  main.py
|  Description :  ServiceNow Service Request Connector Ticket Creation.
...
class SNOWServReqTicketCreation:
	...
	def __init__(self):
		...
		for connector in get_connectors_json['_embedded']['connectors']:
			if connector['type'] == "SNOW_SERVICE_REQUEST" and connector['name'] == self.rs_snow_incident_connector:
		...
		if str(finding_type) == 'host finding':
			...
			if hf_count > 0:
				...
				ticket_form = self.rs.ticket.getcatalogitemfield(self.connectorID)
	...
	def tag_creation(self) -> int:
		...
		tag_name = f"SNOW Service Request Ticket Creation {str(time_obj)}"
	...
	def get_ticket_creation_body(self,ticket_form: dict, connector_id: int) -> dict:
		...
		ticket_api_body = {"connectorId":connector_id,"dynamicFields":[],"type":"SNOW_SERVICE_REQUEST","publishTicketStats":False}
		for field in ticket_form['fields']:
			if field['type'] == 'section':
				for inner_field in field['fields']:
					if inner_field['isSupported'] == True and inner_field['locked'] == False:
						if inner_field['required'] == True:
							required_field_list.append(inner_field)
						elif inner_field['required'] == False:
							optional_field_list.append(inner_field)	
			else:
	...
	def _required_fields_ticket_form(self, required_field_list,ticket_api_body,connector_id):
		for required_field in required_field_list:
			if required_field['type'] == 'textArea':
				...
				required_field_input = input(f"\nEnter the dateTime value for '{required_field['label']}' field. It should be in 'yyyy-mm-dd HH:MM' format: ")
				try:
				...
				while user_satisfied == None:
					...
					query_parameter_body = {"queryParameters":required_field['queryParameters'],"key":required_field['key'],"connectorId":connector_id,"type":"SNOW_SERVICE_REQUEST","query":search_value}
			...
			if optional_field['type'] == 'textArea':
				if optional_field['key'] != 'description' and optional_field['key'] != 'short_description':
					...
					if optional_field_input != 'yes' and optional_field_input != 'no':
						...
						ticket_api_body['dynamicFields'].append(summary_description_field_dict)							 		
			elif optional_field['type'] == 'dateTime':
				optional_field_input = input(f"\nEnter the dateTime value for '{optional_field['label']}' field. It should be in 'yyyy-mm-dd HH:MM' format: ").strip()
				try:
				...
				while user_satisfied == None:
					...
					query_parameter_body = {"queryParameters":optional_field['queryParameters'],"key":optional_field['key'],"connectorId":connector_id,"type":"SNOW_SERVICE_REQUEST","query":search_value}
					query_parameter_json = self.rs.connectors.search_query_parameter(query_parameter_body)
					if len(query_parameter_json['selectOptions']) > 0:
						for ind in range(len(query_parameter_json['selectOptions'])):
							print(f"Index : {ind} - Value : '{query_parameter_json['selectOptions'][ind]['displayValue']}'")
