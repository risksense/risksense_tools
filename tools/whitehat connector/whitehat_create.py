...
class whitehatcreate:
    ...
    def __init__(self):
        ...
        self.whitehatapikey=config['whitehat_api_key']

        self.rs.set_default_client_id(self.client_id)

        try:
            connector_details=self.rs.connectors.create_whitehat(self.connector_name,self.connectorurl,self.scheduletype,self.networkid,self.username,self.whitehatapikey,hour_of_day=0,create_asset=True)
