...
import progressbar
import upload_to_platform.packages.risksense_api as rsapi
...
class UploadToPlatform:
    ...
    def __init__(self, rs_platform, api_key, file_path, log_folder, auto_urba, client_id, network_id):
        ...
        #  Read the config     ***** REMOVED FOR INTEGRATION INTO L&G upload *****
        ...
        if num_upload_errors < len(files):
            ...
            logging.info("There were no files that were successfully uploaded.  Exiting.")
            print()
            exit(1)
        ...
        #     if process_state in ["COMPLETE", "ERROR", "PARSE_FAILED"]:
        #         break
        #     else:
    ...
    def begin_upload_processing(self, upload_id, auto_urba):
        ...
        try:
            ...
            print(" **                                                                             **")
            print(" **  You can check the status of this upload by manually logging in to the      **")
            print(" **  RiskSense platform at any time, and navigating to \"Uploads\".               **")
            print()
