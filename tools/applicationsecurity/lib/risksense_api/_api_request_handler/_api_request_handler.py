...
class ApiRequestHandler:
    ...
    def make_request(self, http_method, url, header=None, params=None, body=None, files=None):
    ...
    def _request_and_validate(self, req_func, **func_params):
        ...
        :raises RequestFailed:
        :raises InsufficientPrivileges:
        :raises NoMatchFound:
        :raises UserUnauthorized:
        """
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 419:
                    raise InsufficientPrivileges(self.__insufficient_privileges(response))

                if response.status_code == 401:
                    raise UserUnauthorized("This user is not authorized.")

                if self.__check_for_page_size_error(response):
                    raise PageSizeError("Maximum page size must be less than or equal to 1000.")

                if self.__check_for_no_match_error(response):
                    raise NoMatchFound("The filter provided did not match any records.")
    ...
    @staticmethod
    def __check_for_no_match_error(response):
        ...
        no_match_error_message = "The filter provided did not match any records"

        if response.status_code == 400 and no_match_error_message in response.text:
            no_match_error = True

        return no_match_error

    @staticmethod
    def __insufficient_privileges(response):

        jsonified_response = json.loads(response.text)
        clients = ", ".join(jsonified_response['clientsMissingPrivileges'])

        exception_string = f"Missing \"{jsonified_response['missingPrivilege']}\" privilege on client(s) {clients}."
