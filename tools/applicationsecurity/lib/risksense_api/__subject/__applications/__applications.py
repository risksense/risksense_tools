...
class Applications(Subject):
    ...
    def get_single_search_page_sortfield(self, search_filters, page_num=0, page_size=150,
                               sort_field=[], client_id=None):
        ...
        try:
            response = self._get_single_search_page_withsort(self.subject_name, **func_args)
        except RequestFailed:
            raise
